...
For recipes, a task is a draw: a function of a key returning `(x, y)`. `junta(n_in, n_out, k)` draws
a fresh junta per key, `addition(n_in)` returns the same table whatever the key. The arithmetic
*family* (addition under random operand permutations, the held-out arithmetic of the reference implementation) enters when a
step needs held-out tasks, at step 2. Every task is exact-width;
