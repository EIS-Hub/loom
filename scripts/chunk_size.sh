...
# claims, probes, prose, lockfiles, generated json). Fails above the cap so a chunk stays readable in one sitting.
set -euo pipefail
base="${1:?base ref}"; cap="${2:-150}"
n=$(git diff --numstat "$base...HEAD" -- . ':!tests/**' ':!claims/**' ':!probes/**' ':!*.md' ':!*.json' ':!*.lock' \
