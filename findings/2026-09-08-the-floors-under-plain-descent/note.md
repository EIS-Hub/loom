...
| hard relay to the logit (straight-through) | both | addition [100, 233]; junta [0.5, 500] | 150 (`HARD_FLOOR`) | 1.0 → 0.8 | one vote moves an entry by a logit; "the bits want a smaller step" was a narrower band |
...
| hard relay to the logit (straight-through) | short, well clear of chance | 0.94 ± 0.01 at 250; 0.90–0.94 on [175, 500]; chance ≤ 75 and ≥ 7500 | 250 (`DEEP_HARD_FLOOR`) | 3.2 → 1.3 | [37, 375] |
...
step at the output. The hard relay to the logit moves an entry by a logit per vote: a bit fabric's natural
...
rate reaches". (2) **The hard relay to the logit trains the bits at four layers** (never under Adam), while the
...
relay without σ′ fails". The same factor cuts the other way on the soft pass, where the relay to
the entry reaches on all six tiles and the reference does not: σ′ = T(1 − T) vanishes as an entry
commits, so with it kept a nearly committed entry stops receiving credit and a run can park a bit
or two short (soft 1.000, hard 0.992, gradient zero), while without it the vote keeps its size and
the last bits get pushed. One factor: an obstacle on the soft pass, an inertia on the bits, where a
committed entry resists a lone "flip" vote (a reading, unmeasured). (3)
...
(`2026-09-09-descent-with-a-readout-at-depth.py`): on the deep tile with the relay to the
