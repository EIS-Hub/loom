...
| 3 | 1 | Signals II: the relay and the uniform transports as `via` coordinates, dropping the surrogate; the ladder scored against the reference (sparsity, cosine, sign agreement) on a shape where depth is forced; `docs/signals.md` extended with the transports; the second note | science | 40 min | 2 | queued |
