...
| 5 | 3 | The rule: a small shared g over the three factors, replacing the product in the same workshop; value-awareness at the rule level as an ablation: g reads the fine signal alone, then the coarse per-gate signal (`signals.errors`) with its own inputs and output, and must reconstruct the address itself | science | 40 min | 5 | queued |
