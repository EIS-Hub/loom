...
| 2 | 1 | Signals I: the read mode as an axis of the combinatorial test (soft, straight-through on the hard tables); what a local update may read (nothing, task identity, error) | science | 20 min | 1 | open (#3) |
| 3 | 1 | Signals II: how error travels, the uniform adjoint and the relay through each gate's own table; the check that the relay keeps the gradient's sign where the uniform adjoint loses it; `docs/signals.md` (the taxonomy: which transport, on which pass, with or without the surrogate factor) and the docs-per-module gate | science | 40 min | 2 | queued |
