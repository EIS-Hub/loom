...
| 1 | Signals: what a local update may read and how error travels; a signal as coordinates (pass × transport × surrogate); descent on the bits; the deploy gap through training | on the soft pass the relay is the reference and reaches the target where the blind split stalls; on the bits, straight-through reaches on a flat tile with no deploy gap at any step, and at depth the exact relay fails where the blind split learns by feedback alignment |
