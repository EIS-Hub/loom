...
| 2 | Meta-learning: the rule (`rule.py`: Δ = −η·s, η the only parameter, never negative, the stored logit optionally a bounded counter) learned from rollouts of itself (`meta.py`: K steps of the rule in one scan; the objective the soft loss of the tile the rule ends on, its deployed loss recorded beside it; the meta-gradient through the K steps, first-order by default on the hard pass; Adam on the host); fresh states first, then a pool of tile states of every age; the tables are never meta-learned; held-out tasks from a key range fixed here; the online window | online, where the objective has an optimum in η, the outer loop finds it from a non-functional start under the true signal (the learned η on the floor of the swept landscape, the non-learned baseline on the same objective), walks η down under the sign flip, parks it where nothing moves under the shuffled and the output-only signals, and the η found adapts a fresh tile to a held-out task; on the bits η is a threshold and the logit a bounded counter |
